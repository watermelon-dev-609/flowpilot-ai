...
| `FlowPilot_AI_开发文档.md` | 技术文档 | 后端 / 前端实现、接口、数据结构等技术细节 |
...
| 阶段 | 模块 | 目标 | 完成标准 | 状态 |
|---|---|---|---|---|
| P0 | 项目骨架 | 搭建前后端基础结构 | 页面可打开，API 可访问，基础布局完成 | 未开始 |
| P1 | 产品中心 | 创建产品、上传图片、填写资料 | 能保存产品基础信息和图片信息 | 未开始 |
| P2 | AI 产品理解 | 生成结构化产品卡片 | 输出功能、场景、技术实体、未知信息 | 未开始 |
| P3 | GEO Research | 生成实体图、用户问题、搜索意图 | 输出 Entity Map 和 Question Map | 未开始 |
| P4 | 内容生成 | 生成 GEO 产品文章 | 文章结构符合 GEO 要求 | 未开始 |
| P5 | Fact Check | 检查事实、参数、未知信息 | 标出 unsupported claim | 未开始 |
| P6 | GEO Critic | 审核 AI 腔、关键词堆砌、结构问题 | 输出评分和修改建议 | 未开始 |
| P7 | AI 引用准备度 | 检查结构化内容、FAQ、实体、爬虫可访问性 | 输出 AI Citation Readiness Score | 未开始 |
| P8 | 多平台适配 | 生成官网、公众号、知乎、百家号、小红书版本 | 每个平台内容差异明显 | 未开始 |
| P9 | 进度与数据记录 | 保存任务历史、生成记录、耗时 | 可查看项目进度和内容历史 | 未开始 |
| P10 | 验收运行 | 实际运行项目并逐项验证 | 所有验收项通过 | 未开始 |
