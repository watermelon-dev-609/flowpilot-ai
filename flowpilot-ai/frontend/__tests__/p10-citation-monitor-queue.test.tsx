...
describe("P10 引用准备度加入监测准备", () => {
  it("生成评分后可以加入监测准备队列", async () => {
    localStorage.clear();
    ...
    expect(await screen.findByText("已加入监测准备队列")).toBeInTheDocument();
    expect(screen.getByText("监测准备队列")).toBeInTheDocument();
    expect(screen.getAllByText("武汉智能沙盘引用准备度检查").length).toBeGreaterThanOrEqual(1);

    const rawQueue = localStorage.getItem("flowpilot.citationReadiness.monitorQueue");
    expect(rawQueue).toBeTruthy();
    const queue = JSON.parse(rawQueue || "[]") as Array<{ topicTitle: string; targetBrand: string; readinessScore: number }>;
    expect(queue[0]).toMatchObject({
      topicTitle: "武汉智能沙盘引用准备度检查",
      targetBrand: "武汉微艺达智能科技有限公司"
    });
    expect(queue[0].readinessScore).toBeGreaterThanOrEqual(70);
  });

  it("未生成评分时不允许加入监测准备", async () => {
    localStorage.clear();
    render(<CitationReadinessPage />);

    expect(screen.getByRole("button", { name: "加入监测准备" })).toBeDisabled();
  });
});
