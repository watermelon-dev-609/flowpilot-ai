...
- 新增 `backend/app/auth.py`：
  - `AuthService` 负责默认管理员初始化、登录、当前用户查询与退出。
  - 密码使用 PBKDF2-SHA256 + 随机盐哈希，数据库不保存明文密码。
  - 会话使用 Bearer Token，过期会话会被拒绝并清理。
- 新增 `AuthUserRecord` 与 `AuthSessionRecord` SQLAlchemy 模型：
  - `auth_users` 保存用户、角色、激活状态与密码哈希。
  - `auth_sessions` 保存 token、用户关联与过期时间。
- 新增 API：
  - `POST /api/auth/login`
  - `GET /api/auth/me`
  - `POST /api/auth/logout`
- 新增 `tests/test_s2_auth.py`，覆盖：
  - 默认管理员初始化与正确密码登录。
  - 错误密码拒绝。
  - 过期 token 拒绝。
  - 密码哈希不保存明文。
  - API 登录、查询当前用户、退出完整流程。
...
- 当前完成的是鉴权核心闭环，尚未把所有写操作端点强制挂上权限依赖；下一段需要逐批保护写接口并同步前端登录态。

下一步：

- S2 第二段：将规则、内容日历、发布队列、GEO 监测与报告快照的写操作接入后端权限校验，并补未登录 / 越权测试。
